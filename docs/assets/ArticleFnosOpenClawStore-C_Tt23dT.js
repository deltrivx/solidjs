...
✅ 更新后自动刷新渠道插件与模型目录
✅ 完整备份可一键还原或新机复用
✅ 整体路径清晰，便于备份、恢复和迁移</pre><p>这套方案的关键不是“把 OpenClaw 跑起来”，而是让它符合 FnOS 商店应用的运行范式：程序归 App Center，数据归 @apphome，权限归独立用户，Gateway 不直接暴露，升级逻辑有顺序、有日志、可回滚。这样迁移到另一台设备时，只需按目录、用户、wrapper、systemd、更新逻辑逐项复刻，就能得到一致的运行环境。`);function s(){return o(()=>{document.querySelectorAll(".reveal").forEach(e=>e.classList.add("visible")),window.scrollTo(0,0)}),(()=>{var e=c(),p=e.firstChild,a=p.firstChild;return n(p,t(r,{href:"/articles",class:"back-link",children:"← 返回文章列表"}),a),e})()}export{s as default};
