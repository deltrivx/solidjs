...
const statusColors = { online: '#00cec9', busy: '#fd79a8', offline: '#8888a0' };
  ...
  return (
    <div class="agent-card tilt-card">
      <div class="agent-card-header">
        ...
        <span class="agent-status-dot" style={{ 'background-color': statusColors[props.status] || '#888' }}></span>
      </div>
      <div class="agent-card-body">
        <h3>{props.name}</h3>
        <p class="agent-title">{props.title}</p>
        <span class="agent-status-label" style={{ color: statusColors[props.status] || '#888' }}>
          {statusLabels[props.status] || props.status}
        </span>
      </div>
      <div class="agent-card-footer">
        <span>📋 {props.tasks ?? 0} 任务</span>
